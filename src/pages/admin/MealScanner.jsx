...
const MealScanner = () => {
    ...
    const processCode = async (code) => {
        ...
        try {
            const res = await mealService.scanReservation(code);
    ...
    useEffect(() => {
        // Auto-start scanning when component mounts (wait for DOM)
                ...
                <Card sx={{ mb: 2, border: '2px solid green' }}>
                    <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h6">
                            {scanResult.user?.name || scanResult.user?.fullName}
                        </Typography>
                        <Typography color="textSecondary" gutterBottom>
                            {scanResult.user?.email}
                        </Typography>
                        <Typography variant="body1" sx={{ mt: 2, fontWeight: 'bold' }}>
                            {scanResult.menu?.mealType === 'LUNCH' ? 'Öğle Yemeği' : 'Akşam Yemeği'}
                        </Typography>
                        <Typography variant="body2" gutterBottom>
                            {scanResult.menu?.date}
                        </Typography>
                        {scanResult.isFree ? (
                            <Typography variant="body2" color="success.main" sx={{ mt: 1, fontWeight: 'bold' }}>
                                Ücretsiz (Burslu Öğrenci)
                            </Typography>
                        ) : (
                            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                                {scanResult.amount ? `${scanResult.amount} TL bakiyeden düşüldü` : ''}
                            </Typography>
                        )}
                    </CardContent>
                </Card>
